...
public class PlaceUILeftPanelController : MonoBehaviour
{
    ...
    */
    ...
    [SerializeField] private ResetPlaceButtonFunction resetPlaceButtonFunction;//배치 초기화 기능을 담당하는 스크립트.


    private void OnEnable()//캔버스 활성화 시 버튼에 리스너 추가.
    {
        placeButtonFunction.enabled = true;//배치 버튼 컴포넌트는 기본으로 활성화되어 있다.
        autoPlaceButtonFunction.enabled = false;//자동 배치 컴포넌트는 기본적으로 비활성화. 
        resetPlaceButtonFunction.enabled = false;//초기화 컴포넌트는 기본적으로 비활성화.
        AddListenerToButtons();
    }

    private void OnDisable()//씬 종료 시 두 컴포넌트는 다시 비활성화 상태로.
    {
        placeButtonFunction.enabled = true;//배치 버튼 컴포넌트는 기본으로 활성화되어 있다.
        autoPlaceButtonFunction.enabled = false;//자동 배치 컴포넌트는 기본적으로 비활성화. 
        resetPlaceButtonFunction.enabled = false;//초기화 컴포넌트는 기본적으로 비활성화.
    ...
    private void DetachListeners()
    {
        ...
        resetPlaceButton.onClick.RemoveAllListeners();
    }
    ...
    {
        if(!resetPlaceButtonFunction.enabled)
        {
            resetPlaceButtonFunction.enabled = true;
        }
        else
        {
            resetPlaceButtonFunction.StartResetPlace();
        }
