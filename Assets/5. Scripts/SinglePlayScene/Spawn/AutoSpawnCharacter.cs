...
public class AutoSpawnCharacter : MonoBehaviour
{
  // SinglePlayScene에 처음 진입 시 자동으로 캐릭터터 유닛이 스폰되도록 하는 스크립트.
    ...
    {
        //플레이어의 성별과 직업에 맞는 캐릭터 프리팹을 가져오는 메서드.
        ...
        playerCharacter = playerCharacterManager.SetCharacterProfile();//성별과 직업에 맞는 플레이어 캐릭터를 설정한다.
        Debug.Log($"playerCharacter is {playerCharacter.name}");

    }

    private void FindSpawnPosition()//캐릭터 프리팹을 타일맵 위 캐릭터터 위치에 스폰하는 메서드.
    {
