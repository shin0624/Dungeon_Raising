...
public class JobSelectManager : MonoBehaviour
{
    ...
    void Start()
    {
        DeactiveAllPanels(maleJobPanels);//하이어라키에 존재하는 남/여 직업별 패널을 모두 비활성화
        DeactiveAllPanels(femaleJobPanels);
    ...
    {
        DeactiveAllPanels(panels);//만약 활성화된 패널이 존재한다면 닫는다. 초기에는 모든 패널이 비활성화 되어 있고, ActiveJobPanel에서는 내가 선택한 성별의 패널만 다루므로, 매개변수로 전달받은 내 성별의 패널만 다시한번 비활성화 체크를 수행. 선택하지 않은 성별의 패널은 계속 비활성화 상태로 남아있음.
        if(panels.Length >0)
        {
    ...
    private void DeactiveAllPanels(GameObject[] panels)//열려있는 직업 패널을 비활성화.
    {
        if (panels == null || panels.Length == 0)//패널이 null인지 먼저 확인.
        {
            Debug.LogWarning("No job panels assigned.");
            return;
        }
    ...
    private void SetJobPanel(string text)//성별에 따라 직업 패널을 설정하는 메서드. 인자로는 genderValue값을 전달받음.
    {
        switch(text)
        {
            case "Male":
